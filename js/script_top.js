...
$('.nic_1').click(function() {	
  let $this = $(this);
  let isEditing = $this.hasClass('nic_1_1');
  
  if (!isEditing) {
    let val = $this.html();
  
    let $input = $('<input type="text" class="nic_1-input">');
    $input.blur(function() {
      let $this = $(this);
      let val = $this.val();
      $this.closest('.nic_1_1').html(val).removeClass('nic_1_1');
    })
    $input.val(val);
    $this.html($input);
    $input.focus();
  }
  $this.addClass('nic_1_1');
});

$('.nic_2').click(function() {	
  let $this = $(this);
  let isEditing = $this.hasClass('nic_2_2');
  
  if (!isEditing) {
    let val = $this.html();
  
    let $input = $('<input type="text" class="nic_2-input">');
    $input.blur(function() {
      let $this = $(this);
      let val = $this.val();
      $this.closest('.nic_2_2').html(val).removeClass('nic_2_2');
    })
    $input.val(val);
    $this.html($input);
    $input.focus();
  }
  $this.addClass('nic_2_2');
});

$('.nic_3').click(function() {	
  let $this = $(this);
  let isEditing = $this.hasClass('nic_3_3');
  
  if (!isEditing) {
    let val = $this.html();
  
    let $input = $('<input type="text" class="nic_3-input">');
    $input.blur(function() {
      let $this = $(this);
      let val = $this.val();
      $this.closest('.nic_3_3').html(val).removeClass('nic_3_3');
    })
    $input.val(val);
    $this.html($input);
    $input.focus();
  }
  $this.addClass('nic_3_3');
});
